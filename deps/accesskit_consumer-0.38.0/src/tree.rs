...
impl State {
    ...
    fn update(
        ...
        // PLEXI PATCH: a root change can orphan the node named by the update's
        // focus after the subtree sweep. Fall back to the live root rather than
        // panicking in validate_global with a stale focus ID.
        if !self.nodes.contains_key(&self.focus) {
...
mod tests {
    ...
    #[test]
    fn root_change_with_focused_orphan_falls_back_to_new_root() {
        let first_update = TreeUpdate {
            nodes: vec![
                (LocalNodeId(0), {
                    let mut node = Node::new(Role::Window);
        ...
        let second_update = TreeUpdate {
            nodes: vec![(LocalNodeId(2), Node::new(Role::Window))],
            tree: Some(Tree::new(LocalNodeId(2))),
            tree_id: TreeId::ROOT,
            focus: LocalNodeId(1),
        };
        tree.update_and_process_changes(second_update, &mut NoOpHandler);

        assert_eq!(tree.state().focus().map(|node| node.id()), Some(node_id(2)));
    }
