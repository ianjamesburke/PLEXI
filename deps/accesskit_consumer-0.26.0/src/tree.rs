...
impl State {
    ...
    fn update(
        ...
        // If the focused node was removed (e.g. as an orphan during a root
        // change), fall back to the root rather than panicking in validate_global.
        if self.nodes.get_key(&self.focus).is_none() {
            self.focus = self.data.root;
        }
