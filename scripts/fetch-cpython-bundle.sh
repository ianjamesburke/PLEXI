...
STDLIB_ARCHIVE="Python-${VERSION}.tgz"
STDLIB_URL="https://www.python.org/ftp/python/${VERSION}/${STDLIB_ARCHIVE}"
STDLIB_SHA256="487c908ddf4097a1b9ba859f25fe46d22ccaabfb335880faac305ac62bffb79b"
...
if [[ -f "$PYTHON_WASM" ]] && [[ -f "$VERSION_FILE" ]] && [[ -f "${BUNDLE_DIR}/Lib/encodings/__init__.py" ]]; then
...
curl -fL --progress-bar "$STDLIB_URL" -o "${tmp}/${STDLIB_ARCHIVE}"
...
stdlib_actual="$(sha256_file "${tmp}/${STDLIB_ARCHIVE}")"
if [[ "$stdlib_actual" != "$STDLIB_SHA256" ]]; then
    echo "error: stdlib SHA256 mismatch for ${STDLIB_ARCHIVE}" >&2
    echo "  expected: ${STDLIB_SHA256}" >&2
    echo "  actual:   ${stdlib_actual}" >&2
    exit 1
fi
...
tar -xzf "${tmp}/${STDLIB_ARCHIVE}" --strip-components=2 -C "${BUNDLE_DIR}/Lib" "Python-${VERSION}/Lib"
