...
**Status:** Proposal  
**Target:** v2.x (post-v2.0 orchestration layer)  
...
### Phase 1 — `--embedded` Flag (Proves Recursive Nesting)
Add `--embedded` flag to the Plexi binary. When set, render to PGAP stdout instead of an OS window. Register as a launchable app. Proves the recursive nesting model works end-to-end with a real binary speaking PGAP.

### Phase 2 — Python App Hot Reload
File watcher + `Shutdown`/`Init` cycle. Crash-on-invalid-state with helpful error. State persisted to `.plexi/` directory. Tracebacks forwarded through app stderr log pipeline.

### Phase 3 — Render Modes & StatusSummary
Add `mode` field to `Render` event. Implement `"preview"` render mode. Add `StatusSummary` draw command. Implement the three preview levels (status-only, single-depth wireframe, double-depth miniature).

### Phase 4 — TreeStatus, Notifications, Bookmarks
Implement `TreeStatus` rollup propagation. Add depth addresses to notifications. Implement jumpable notifications via direct pipe promotion. Add `bookmarks.toml` storage and resolution.

### Phase 5 — Capability Manifest & Secret Broker
Implement capability manifest in `Init` event. Build the root-resident secret broker with `RequestSecret` PGAP command. Add TTL watchdog. Implement capability attenuation enforcement for grandchild spawning.

### Phase 6 — Direct Pipe Promotion
Implement `Suspend`/`Resume` events. Build the direct pipe bypass in the root so focused-depth communication has zero intermediate overhead.
