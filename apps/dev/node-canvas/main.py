...
from plexi_sdk.ui import TEXT_CAPTION, TEXT_HINT
...
class NodeCanvas(App):
    ...
    def _draw_node(self, ctx: RenderContext, node: Node) -> None:
        ...
        border = ctx.theme.accent if selected else C_NODE_BORDER

        # shadow
        ctx.rect(sx + 3, sy + 3, node.w, node.h, "#00000044", radius=6.0)
        # body
        ctx.rect(sx, sy, node.w, node.h, ctx.theme.surface, radius=6.0)
        # header
        ...
        # title
        ctx.text(sx + PORT_LABEL_X, sy + (NODE_HEADER_H - TEXT_CAPTION) / 2,
                 node.title, size=TEXT_CAPTION, color=ctx.theme.fg, bold=True,
                 max_width=node.w - PORT_LABEL_X * 2, elide=True)

        # input ports
        for i, port in enumerate(node.inputs):
            py = sy + NODE_HEADER_H + i * PORT_H + PORT_H / 2
            ctx.circle(sx, py, PORT_R, ctx.theme.green)
            ctx.text(sx + PORT_R + 4, py - TEXT_HINT / 2, port.name,
                     size=TEXT_HINT, color=ctx.theme.muted, max_width=node.w / 2 - PORT_R - 8, elide=True)
    ...
    def _draw_hint(self, ctx: RenderContext) -> None:
        ...
        ctx.text(8, ctx.h - TEXT_HINT - 6, hints, size=TEXT_HINT, color=ctx.theme.muted)
