...
class DescriptorRendererApp(App):
    ...
    def on_render(self, ctx: RenderContext) -> None:
        ...
        if self._view == "error":
            ctx.render(Column([
                Spacer(SPACE_LG),
                Card([
                    Label("Error", tone="danger", bold=True),
                    Spacer(SPACE_SM),
    ...
    def _render_form(self, ctx: RenderContext) -> None:
        ...
        if not self._form_fields:
            run = ListItem(title="▶  Run", background=ctx.theme.accent, selected=False)
        ...
        if self._last_run:
            ...
            ctx.rect(SPACE_LG, y, ctx.w - 2 * SPACE_LG, last_run_h, ctx.theme.highlight, radius=RADIUS_SM)
            ctx.text(x=SPACE_LG + SPACE_SM, y=y + last_run_h / 2,
                     text=f"$ {self._last_run}",
                     size=TEXT_HINT, color=ctx.theme.muted, monospace=True,
