...
- **Every top-level app is a maintained exemplar.** The directories directly under `apps/` (not `dev/`, not `examples/`) are the curated exemplar set: `balls`, `breakout`, `calc`, `csv_viewer`, `github-issues`, `kraken`, `logs`, `permissions`, `snake`, `stats`, `tetris`, `todo`, `wikipedia`. [`packs/core.toml`](../packs/core.toml) is the subset the host auto-seeds into every profile on launch (`logs`, `stats`, `todo`, `calc`, `permissions`); other exemplars seed on demand. Everything under `dev/` and `examples/` is a throwaway proof-of-concept. Touch a `dev/` app only when the change demonstrates a new SDK or host capability.
