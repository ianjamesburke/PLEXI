"""V3 example: an AI coding assistant using module-level app functions."""

from plexi_sdk import state
from plexi_sdk.effects import AiMessage, AiQuery, SetState, SetTitle
from plexi_sdk.events import AiResponse, UiAction, UiValueChange
from plexi_sdk.ui import AppBar, Button, Column, FooterKeys, Label, TextEdit

SYSTEM_PROMPT = "You are a concise coding assistant. Explain fixes and tradeoffs clearly."


def init(_size, _args):
    return [SetTitle("Coding Agent"), SetState({"prompt": "", "answer": ""})]


def update(event):
    if isinstance(event, UiValueChange) and event.handler_id == "prompt":
        return [SetState({"prompt": event.value})]
    if isinstance(event, UiAction) and event.handler_id in ("prompt", "ask"):
        prompt = state.get("prompt", "").strip()
        if prompt:
            return [AiQuery("coding-agent-query", "medium", SYSTEM_PROMPT,
                            [AiMessage("user", prompt)])]
    if isinstance(event, AiResponse) and event.request_id == "coding-agent-query":
        return [SetState({"answer": event.error or event.content or "No response."})]
    return []


def view():
    return Column([
        AppBar("Coding Agent", subtitle="V3 AI-query example"),
        TextEdit("prompt", value=state.get("prompt", ""),
                 placeholder="Paste code or describe the problem...", multiline=True),
        Button("Ask", on_click="ask", style="primary"),
        Label(state.get("answer", "Ask for a review, debug plan, or explanation."), max_lines=20),
        FooterKeys([("enter", "submit text"), ("ask", "send query")]),
    ], grow=True)
