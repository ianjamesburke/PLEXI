...
- **One worktree, one PR.** Claim every given task, implement all of them in a single worktree branched from alpha HEAD, and open ONE PR to alpha covering the whole batch. Branch: the multi-id form `feature/stint-<id>[-<id>...]-<short-slug>` with **every** batched task id in the name — `just merge-pr` resolves the stints to close from the branch name plus the PR **body** (the title is never read), with the branch as primary carrier. Every batched id must also appear in the PR body; `/open-pr`'s stint-first mode does that.
