...
## Current State (2026-07-19, mimo run forensics)

`alpha` is at `fdaf2fe2` (PRs #2442/#2443, stints `0468`/`0467`), from forensics of the first mimo-tier app build (tictactoe, 6 tool calls — the model works on the medium tier now that signatures are injected): the `app check` action probe discovers and clicks real handlers in one guest session instead of a one-entry allowlist (the tictactoe first-click crash now fails check), `view()` returning a tuple/effect raises a teachable contract error instead of "Unknown UINode type", and the assistant transcript is never frozen — a `⠹ writing host.files.write · 3.4k chars · 12s` row animates while the model generates tool arguments (previously 46–57s of static text), with thinking dots in every other waiting state. Open question from the run: the `tier: high` floor on `build-plexi-app` may be obsolete now that the reference carries signatures — verify with a mimo build, then drop the floor.

Prior state (earlier 2026-07-19): PR #2441 (stint `0465`): the SDK reference injected into app-build turns carries a real constructor signature for every public class (the generator previously emitted 154 near-empty headings — the ant-simulator run guessed `SetTimer(ms=...)` and burned 6 recovery calls spelunking example apps), the app-build skill forbids example-app discovery, and the host injects an "App placement" line so `app init` passes `--global` correctly on the first call. Expected floor for a clean build: 4 tool calls (init, write, check, open).

Prior state (earlier 2026-07-19): PR #2440 fixed everything the first user-driven fish-game run exposed: `app check` fails fast on a broken guest — ~2s instead of 76s of silent 15s-timeout spins (`0458`); app-build turns stop grepping the SDK (the embedded reference is authoritative) and escalate off a default-sourced weak tier via skill-frontmatter `tier: high` (`0459`); one Escape press interrupts an in-flight turn on both the live and CLI key paths, the running tool row animates with elapsed time, tool caret bodies show labeled full-width in/out blocks with real newlines, and OpenRouter failures surface the provider's actual error (`0460`) — which found and fixed the real bug behind "model switch ignored": anthropic-family providers reject dotted tool names, now encoded to wire-safe form. New: `plexi host screenshot [--pane <id>]` captures live host pixels through the render pipeline — the only sanctioned capture path, documented in root AGENTS.md (`0461`). Validated live on the PR build: same fish prompt went from ~45 tool calls / ~8 min to **8 tool calls / ~3 min**, game verified playing via pane-cropped screenshots. Launch gate (`0413`): ready for the next user-driven alpha run. Follow-up `0457` still annotates the legacy maintained apps for a strict type gate.
