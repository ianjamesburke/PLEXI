...
2. **Wait for progress, event-driven.**
   ...
   **Watch the pane and the PR in the same loop.** A lane has two independent failure surfaces and pane state only covers one. CI can go red while the pane sits legitimately idle, and a loop polling only slots will not see it. Once a `pr` slot exists, every wait iteration checks `gh pr checks <PR#>` too, and a red check wakes the head exactly like a terminal slot does — **but only for a commit the head has not already ruled on.** Record the head sha when you arm the watch and ignore CI until it changes; the red that caused a fix round stays on the wire for the whole round, so a naive check fires instantly and every time, which is the same as not watching at all.
