...
1. **Worker: implement + open PR.** The brief is always the skill invocation plus a freeform overlay — never a restatement of mechanics:
   ...
   **If a worker is chasing `.plexi-<channel>` / `config_dir` / `temp_dir` test failures**, that is the host-env leak Worker Mode's env-stripped test command exists to prevent — tell it so and point it back at that command. (Permanent fix tracked in the test-hermeticity stint; once that lands, `just test` strips these itself.)
