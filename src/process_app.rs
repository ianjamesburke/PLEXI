...
fn find_python() -> std::ffi::OsString {
    ...
    log::warn!(
        "process_app: no Python >= 3.10 found at known paths;          falling back to `python3` (may be system 3.9 in GUI context).          Install Python 3.10+ via: brew install python@3.13"
    );
    ...
    fn ui(&mut self, ui: &mut egui::Ui, ctx: &AppRenderContext<'_>) {
            ...
            match cmd {
               ...
               DrawCommand::State { user_state, derived, session, persistent } => {
                ...
                DrawCommand::SpawnApp {
                    ...
                    log::debug!(
                        target: &target,
                        "spawn_app requested: target={app_id} parent={parent:?} layout={layout:?}                          lifecycle={lifecycle:?} linked={linked} channels={wire_channels:?}"
                    );
                ...
                DrawCommand::MeasureText { request_id, text, size, monospace, bold } => {
                    // Store in pending_frame; resolved to a TextMetrics event in ui().
        ...
        for cmd in measure_cmds {
            if let DrawCommand::MeasureText { request_id, text, size, monospace, .. } = cmd {
                ...
                self.send_event(&PlexiEvent::TextMetrics {
                    request_id,
                    width: text_size.x,
                    height: text_size.y,
        ...
        // Send any pending back-events (RunCreated, TextMetrics, etc.) to the app.
