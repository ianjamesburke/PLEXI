...
/// One incremental token delta surfaced to the `run_turn` caller. Reasoning
/// chunks are carried separately from answer text and never accumulate into
/// `TurnResult::text`.
#[derive(Debug, Clone, Copy)]
pub enum TokenDelta<'a> {
    Answer(&'a str),
    Reasoning(&'a str),
}
...
///
/// `on_token` is called with each answer/reasoning chunk as it arrives. Pass
/// a no-op closure if streaming is not needed (e.g. in tests).
pub fn run_turn(
    backend: &dyn AiBackend,
    request: AiBackendRequest,
    mut on_token: impl FnMut(TokenDelta<'_>),
    ...
    loop {
        match rx.recv() {
            Ok(StreamEvent::Text(chunk)) => {
                on_token(TokenDelta::Answer(&chunk));
                text.push_str(&chunk);
            }
            Ok(StreamEvent::Reasoning(chunk)) => {
                on_token(TokenDelta::Reasoning(&chunk));
            }
