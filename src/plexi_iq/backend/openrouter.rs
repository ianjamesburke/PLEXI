//! OpenRouter backend for the Plexi IQ broker (`iq.query` path, #383).
//!
...
impl LlmBackend for OpenRouterBackend {
    ...
    fn stream_to_channel(
        ...
        thread::Builder::new()
            .name("plexi-iq-openrouter".to_string())
            .spawn(move || {
                stream_openrouter(api_key, model, request, tx);
            })
            .map_err(|e| LlmError::Io(format!("failed to spawn OpenRouter stream thread: {e}")))?;
...
fn stream_openrouter(
    ...
    if request.messages.is_empty() {
        let _ = tx.send(StreamEvent::Error(
            "LlmRequest.messages is empty — backend requires at least one message".to_string(),
        ));
        return;
    }
...
mod tests {
    ...
    fn system_prompt_goes_in_messages_array_not_top_level() {
        // Simulate message building logic used in stream_openrouter.
        let system = "You are a helpful assistant.".to_string();
        let msgs = vec![IqMessage {
            role: "user".to_string(),
