...
`builtin_factory` (`src/pane_ops/create.rs`) — parameterized by one argument:
...
`CliRendererApp::new(descriptor_path)` (`src/render/cli_renderer_app.rs`) is
...
`bad_descriptor_path_yields_error_view` (`src/render/cli_renderer_app.rs`).
...
**first render frame** (`App::ui`, `src/render/cli_renderer_app.rs`), if a
descriptor loaded successfully, the renderer:

1. Calls `request_terminal`, which enqueues an
...
The host drains that command in `PlexiApp` (`src/app/mod.rs`), which calls
`dispatch_request_linked_terminal` (`src/app/canvas_bindings.rs`). That
...
The renderer receives that event in `queue_outbound_event`. If the
...
> `LinkedTerminalReady { terminal_pane_id: 0 }` (`canvas_bindings.rs`)
...
The host drains it (`src/app/mod.rs`) into
`dispatch_run_in_linked_terminal` (`src/app/canvas_bindings.rs`), which
...
pane-zoom toggle (verified by
`cmd_enter_passes_through_for_pane_zoom`).
...
> (`canvas_bindings.rs` doc comment).
...
`restore_state` is a no-op (–), so a CLI-backed pane has **no
...
`$TMPDIR/plexi-descriptor-<uuid>.json` (`src/cli/open.rs`). It is unique per
...
`crawl.rs`) — `~/.plexi/`, `~/.plexi-alpha/`, `~/.plexi-pr-<N>/`, etc. See §6.

**Invalidation is by CLI version** (`crawl_with_runner`, `crawl.rs`):
...
capability check (`canvas_bindings.rs`). There are zero permission or
...
- **The crawl cache path** is derived from `crate::config::config_dir`
 (`crawl.rs`), so each channel caches into its own profile dir
...
| `RequestLinkedTerminal: pane <s> → terminal <t>` (`canvas_bindings.rs`) | Host created the linked terminal |
| `RunInLinkedTerminal: pane <t> ← "<cmd>\n"` (`canvas_bindings.rs`, debug) | Host wrote to the PTY |
| `open:cli: launching cli-renderer for '<name>' with descriptor at <path>` (`open.rs`) | The temp file path used for this open |

Warn-level lines flag the degraded paths: `no linked terminal, cannot run`, `bad JSON in <path>`, `cannot read <path>`,
`RunInLinkedTerminal: ... not linked` (`canvas_bindings.rs`).
...
- The pane's display name comes from `App::display_name`
 (`cli_renderer_app.rs`): the descriptor's `name`, or `"CLI Renderer"` if
 the descriptor failed to load.
- The pane's app type id is `"cli-renderer"` (`App::type_id`).
...
`wants_close` returning `false` means a CLI-backed pane is closed only
...
1. **No reload / re-resolution affordance.** `cli_renderer_app.rs`
...
2. **No manual crawl-cache invalidation.** `src/cli/crawl.rs` invalidates
...
3. **Version-less CLIs never invalidate.** `src/cli/crawl.rs`: when
...
`writes`/`reads` arrays (documented in `CLI_DESCRIPTOR_GUIDE.md` §3 as "trust
 ...
 (`cli_renderer_app.rs`) runs the command unconditionally. There is no
 confirm-before-write prompt. The fields are advisory metadata only.

5. **No command-execution confirmation.** `cli_renderer_app.rs` →
`canvas_bindings.rs`: submitting a form writes straight to the PTY with no
...
6. **Stale temp files accumulate.** `src/cli/open.rs` writes
