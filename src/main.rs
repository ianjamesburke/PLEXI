...
fn main() -> eframe::Result {
    ...
    // index is in the new flat-string form. Host startups only: a keychain
    // value read from a freshly compiled binary fires a credential dialog
    // (macOS ACLs are per-binary), so ephemeral CLI processes — including
    // every test- or gate-spawned invocation — must never run it. An attended
    // host launch is the one place a prompt can be answered.
    #[cfg(target_os = "macos")]
    {
        if !cli_mode {
                ...
                match cmd {
                    ...
                    Commands::Events { cmd } => match cmd {
                        ...
                        EventsCmd::Declare {
                            ...
                            schema,
                            description,
                        } => std::process::exit(cli::events_declare_cli(
                            &app_id,
                            &stream,
                            &schema,
                        ...
                        EventsCmd::Emit {
                            ...
                            rollback_token,
                            changed_resources,
                        } => std::process::exit(cli::events_emit_cli(cli::EmitArgs {
                            app_id: &app_id,
                            event: &event,
                            summary: &summary,
                            resource: &resource,
                            ...
                            actor: &actor,
                            resource_scope: resource_scope.as_deref(),
                            payload: payload.as_deref(),
                            state_ref: state_ref.as_deref(),
                            revision_before: revision_before.as_deref(),
