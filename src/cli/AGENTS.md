...
**Socket rule:** An explicit `--socket` wins, followed by `PLEXI_SOCKET`; otherwise a channel-suffixed binary resolves its own profile endpoint and the bare `plexi` binary has no command socket. Route new command dispatch through `resolve_command_socket()`. On Windows the endpoint is the stable `\\.\pipe\plexi-<channel>` named pipe, never a filename-derived `.exe` suffix.
...
- **Lifecycle consumers share the brokered event connection.** `pane wait` and `pane events --follow` use `app::event_stream` and the existing timeline subscription queue. Register before reading the current snapshot, retain each connection's own delivery identity, and release it on completion or disconnect. CLI-facing predicates and record semantics are documented in `skills/plexi-cli/SKILL.md`; lifecycle records are defined in `host::pane_lifecycle`.
...
- **`pane state` separates the agent's claim from the host's observation, and neither is reachable without the other.** A pane's `status` slot is a claim an agent writes about itself; it is not liveness — a wedged head can sit for hours with that slot still reading `running`. `plexi pane state <id>` therefore always returns `claimed_state` (the `status` slot's raw value plus its age, agent-authored, `null` if never written) and `observed_state` (host-derived: process alive, seconds since last output, child pids via `pgrep -P`, current foreground command) together, computed by the single accessor `crate::host::pane_liveness::compute`. `--stale-after <secs>` raises a typed `stale_claim` when a claim exists but no output has been observed within that window; omitted, no default is invented and `stale_claim` stays `null`. Restart policy and event-bus lifecycle transport are out of scope for this primitive (0628, 0664).
...
- **Lifecycle facts survive the status projection.** `agent report --event` retains the raw provider event separately from working/idle/blocked; `--blocked-reason` carries an explicit typed reason. The shared hook script passes raw events through the invocation-scoped `PLEXI_AGENT_EVENT` variable so older channel CLIs still accept its arguments. Reports without provenance remain supported and are labeled legacy reports. The event contract lives in `src/host/pane_lifecycle.rs`.
