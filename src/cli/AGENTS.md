...
- **Background host launch is explicit boot state.** `plexi host start --background` launches without activating Plexi; on macOS it uses Accessory policy and has no normal Dock or menu-bar presence. The flag is forwarded through a launch-only env var that is stripped from child env before explicit re-add, so background mode never cascades into a nested `host start`. It is not configuration. App Nap exemption remains unconditional. Pane send/key/click and `pane focus` operate inside the host through egui/PTY paths and do not request OS activation; `pane focus` changes focus within Plexi but does not front an Accessory app.
