...
- **Boot state comes from the agent detector, never a PTY regex.** `pane new --agent` blocks on the pane's hook-reported `PaneAgentState` reaching idle (`SetAgentState` → `Pane::agent()`). Do not add a parallel prompt-signature scraper for the same fact.
