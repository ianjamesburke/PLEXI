...
fn consume_lifecycle<R: BufRead, W: Write, E: Write>(reader: R, output: &mut W, errors: &mut E, wait: bool) -> i32 {
    for line in reader.lines() {
        let value: serde_json::Value = match line {
            Ok(line) => match serde_json::from_str(&line) {
                Ok(value) => value,
                Err(error) => { let _ = writeln!(errors, "error: invalid lifecycle reply: {error}"); return 1; }
            },
            Err(error) => { let _ = writeln!(errors, "error: reading lifecycle stream: {error}"); return 1; }
        };
        match value["type"].as_str() {
            ...
            Some("timeout") if wait => { let _ = writeln!(errors, "pane wait timed out"); return 2; }
            Some("error") => { let _ = writeln!(errors, "error: {}", value["message"].as_str().unwrap_or("lifecycle request failed")); return 1; }
            _ => { let _ = writeln!(errors, "error: unexpected lifecycle reply"); return 1; }
        }
    }
...
pub fn pane_lifecycle_wait_cli(pane: u64, predicate: &str, timeout: f64) -> i32 {
    if !matches!(predicate, "idle" | "blocked" | "exited") || !timeout.is_finite() || timeout <= 0.0 {
        eprintln!("error: --until must be idle, blocked, or exited; --timeout must be finite and positive");
        return 1;
    }
    let Some(client_timeout) = std::time::Duration::try_from_secs_f64(timeout).ok()
        .and_then(|value| value.checked_add(std::time::Duration::from_secs(5))) else {
        eprintln!("error: timeout is too large"); return 1;
    };
    lifecycle_request(serde_json::json!({"type":"pane_lifecycle_wait", "pane_id":pane,
        "until":predicate, "timeout":timeout, "from_pane_id":from_pane_id()}), Some(client_timeout))
}

pub fn pane_lifecycle_follow_cli(pane: Option<u64>) -> i32 {
    lifecycle_request(serde_json::json!({"type":"pane_lifecycle_follow", "pane_id":pane,
        "from_pane_id":from_pane_id()}), None)
}

fn lifecycle_request(request: serde_json::Value, timeout: Option<std::time::Duration>) -> i32 {
    let stream = match connect_and_send(&request) { Ok(stream) => stream, Err(code) => return code };
    if let Err(error) = stream.set_read_timeout(timeout) {
        eprintln!("error: setting lifecycle response deadline: {error}"); return 1;
    }
    ...
    consume_lifecycle(BufReader::new(stream), &mut std::io::stdout().lock(), &mut std::io::stderr().lock(), timeout.is_some())
}
...
mod pane_consumer_output_tests {
    ...
    fn pane_consumer_exit_codes_and_stdout_are_branchable() {
        for (input, wait, expected, stdout) in [
            ...
            assert_eq!(consume_lifecycle(input.as_bytes(), &mut out, &mut err, wait), expected);
