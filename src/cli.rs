...
class __CLASS_NAME__(App):
    ...
    def on_render(self, ctx: RenderContext) -> None:
        # ctx.w / ctx.h — current pane dimensions; ctx.elapsed — seconds since last frame
        ctx.clear(BG)

        # ── Header ───────────────────────────────────────────────────────────────
        # ctx.row positions children side-by-side with host-side flexbox.
        # badge_child / text_child / key_chip_child build the children list.
        ctx.row(
            x=PAD, y=PAD,
            children=[
                ctx.text_child("__DISPLAY_NAME__", size=HEADING, color=ACCENT,
                               bold=True, monospace=True),
                ctx.badge_child("v0.1.0", fill=SURFACE, fg=MUTED,
        ...
        ctx.text(PAD, self._btn.y + self._btn.h + PAD_TIGHT,
                 f"Clicks: {self.click_count}", size=BODY, color=ACCENT)

    def on_key(self, ctx: RenderContext, key: str, mods: dict) -> None:
        # key: "a"–"z", "0"–"9", "up"/"down"/"left"/"right", "return", "escape", etc.
      ...
      case $line[1] in
        ...
        terminal)
          _arguments \
            ...
            '--layout[Layout hint]:layout:(split_v split_h split_above tab new_window)' \
        ...
        open)
          _arguments \
            '--layout[Layout hint]:layout:(split_v split_h split_above tab new_window overlay)' \
            '--from-pane-id[Split relative to this pane ID]:pane_id:' \
...
const BASH_COMPLETION: &str = r#"_plexi_completions() {
  ...
  case $cmd in
    ...
    terminal)
      if [[ $prev == "--layout" ]]; then
        COMPREPLY=($(compgen -W "split_v split_h split_above tab new_window" -- "$cur"))
      elif [[ $prev == "--cwd" ]]; then
    ...
    open)
      if [[ $prev == "--layout" ]]; then
        COMPREPLY=($(compgen -W "split_v split_h split_above tab new_window overlay" -- "$cur"))
      else
...
complete -c plexi -n "__fish_seen_subcommand_from terminal" -l layout -d "Layout hint" -a "split_v split_h split_above tab new_window"
...
# open flags
complete -c plexi -n "__fish_seen_subcommand_from open" -l layout -d "Layout hint" -a "split_v split_h split_above tab new_window overlay"
