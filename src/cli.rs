...
This file demonstrates core SDK primitives — delete what you don't need.
"""
...
class __CLASS_NAME__(App):
    async def on_init(self, ctx: RenderContext) -> None:
        ...
        # ctx.workspace_root, ctx.capabilities, and ctx.feature_flags are set.
        # Use async def to await blocking SDK calls; plain def is fine otherwise.
        self.click_count = 0

        # Request a runtime capability — declare it in manifest.toml too.
        # net_ok = await self.emit.capability_request("net.http")
    ...
    def on_render(self, ctx: RenderContext) -> None:
        ...
        # ctx.elapsed is seconds since the previous frame (0.0 on first frame).
    ...
    def on_key(self, ctx: RenderContext, key: str, mods: dict) -> None:
        # key strings: "a"–"z", "0"–"9", "up"/"down"/"left"/"right",
    ...
    def on_click(self, ctx: RenderContext, x: float, y: float, button: str) -> None:
        ...
        # Fire-and-forget: ctx.notify(title, body, priority=PRIORITY_NORMAL)
