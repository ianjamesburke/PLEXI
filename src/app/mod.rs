...
/// apple bindings); Linux answers the same question via `SO_PEERCRED`; Windows
...
impl eframe::App for PlexiApp {
    ...
    fn logic(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        // being painted. This drain lived in `ui` until stint 0688, so an
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        // Stint 0688: app-command servicing moved to `App::logic` — see
        // `service_app_commands`. Only the modal-render follow-up stays here,
        // because `early_modal_cmds` is produced by this pass's overlay draw.
        self.sync_app_cwd();
        ...
        // macOS affordance (`platform::macos_menu` is not compiled
        // elsewhere), so the drain is gated with the menu itself. Config still
        // hot-reloads via the fs watcher below on every platform.
        #[cfg(target_os = "macos")]
        {
