...
//! paths, `..` components, and symlinks (in package content or as archive
//! entries) are all rejected before any byte is written. Root-level generated
//! development artifacts such as `.venv/` are excluded before validation.
...
/// Directory names produced by scaffold/check tooling (env, cache, build
/// output). Single authority shared by packaging (root-level exclusion) and
/// the Assistant's file-tool walks (skipped at any depth) so the two lists
/// cannot drift.
pub(crate) fn is_generated_dev_dir_name(name: &str) -> bool {
    matches!(
        name,
        ".venv"
            | "__pycache__"
...
/// that must not become package content.
pub(crate) fn is_generated_dev_artifact_rel(rel_path: &Path) -> bool {
    ...
    name.to_str().is_some_and(is_generated_dev_dir_name)
}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn generated_root_venv_is_excluded_from_package_content() {
        let work = TempDir::new().unwrap();
        let app_dir = work.path().join("app");
        fs::create_dir(&app_dir).unwrap();
        write_app(&app_dir, "pkg-venv", "");
        let venv_bin = app_dir.join(".venv").join("bin");
        fs::create_dir_all(&venv_bin).unwrap();
        fs::write(app_dir.join(".venv").join("pyvenv.cfg"), "home = /tmp\n").unwrap();
        ...
        assert!(
            entries.keys().all(|path| !path.starts_with(".venv/")),
            "package entries must exclude .venv, got {:?}",
