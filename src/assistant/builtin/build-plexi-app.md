...
---
Build the app as a real Plexi app, never as a loose script run from a terminal. Run every shell command below with host.terminals.run in one reused terminal pane.

1. Scaffold: `plexi app init --global <kebab-name>`. This creates the app directory (printed by the command) with `manifest.toml`, `main.py`, `tests/`, and an `AGENTS.md` authoring guide.
