...
mod tests {
    ...
    use crate::protocol::view::{
        CanvasPrimitive, Color, DocumentNode, DocumentStyle, InlineText, Point, Rect, TextStyle,
        View,
    };
    ...
    #[test]
    fn renders_rect_at_known_coordinates() {
        let renderer = HeadlessRenderer::new();
        let view = View::Canvas {
            primitives: vec![CanvasPrimitive::Rect {
                rect: Rect { x: 10.0, y: 10.0, w: 80.0, h: 60.0 },
                fill: red(),
                radius: 0.0,
            }],
        };
        let pixmap = renderer.render_to_pixmap(&view, 200, 100);

        // Centre of the rect should be solid red (premultiplied, alpha=255 → r=255).
        let px = pixmap.pixels()[40 * 200 + 50]; // (x=50, y=40)
        assert_eq!(px.red(), 255, "red channel");
        assert_eq!(px.green(), 0, "green channel");
        assert_eq!(px.blue(), 0, "blue channel");
        assert_eq!(px.alpha(), 255, "alpha");

        // A pixel outside the rect should be transparent (black, a=0 from empty Pixmap).
        let outside = pixmap.pixels()[5 * 200 + 5]; // (x=5, y=5)
        assert_eq!(outside.alpha(), 0, "outside rect is transparent");
    }
        ...
        let view = View::Canvas {
            primitives: vec![
                CanvasPrimitive::Rect {
                    rect: Rect { x: 0.0, y: 0.0, w: 200.0, h: 50.0 },
                    fill: Color { r: 30, g: 30, b: 30, a: 255 },
                    radius: 0.0,
                },
                CanvasPrimitive::Text {
                    pos: Point { x: 10.0, y: 36.0 },
                    text: "Hello Plexi".to_string(),
                    style: TextStyle { size: 16.0, color: white(), bold: false, italic: false },
                },
            ],
        };
        let png = renderer.render_to_png(&view, 200, 50);
        assert!(!png.is_empty(), "PNG should be non-empty");
        assert!(png.starts_with(b"\x89PNG"), "output is a valid PNG header");
    }

    #[test]
    fn pgap_commands_render_to_png() {
        let renderer = HeadlessRenderer::new();
        let commands = vec![
            serde_json::json!({"type": "rect", "x": 0.0, "y": 0.0, "w": 200.0, "h": 100.0, "fill": "#ff0000", "radius": 0.0}),
            serde_json::json!({"type": "text", "x": 10.0, "y": 50.0, "text": "Agent loop", "size": 14.0, "color": "#ffffff", "monospace": false, "bold": false}),
            serde_json::json!({"type": "frame_done", "frame_id": 1}),
        ];
        let png = renderer.render_pgap_frame(&commands, 200, 100);
        assert!(png.starts_with(b"\x89PNG"), "output must be a valid PNG");

        // Reload and assert: background rect is red at centre.
        let img = image::load_from_memory(&png).expect("valid PNG").to_rgba8();
        let px = img.get_pixel(100, 50);
        assert_eq!(px[0], 255, "red channel must be 255 for #ff0000 rect");
        assert_eq!(px[1], 0, "green channel");
        assert_eq!(px[2], 0, "blue channel");
    }

    #[test]
    fn parse_hex_color_css_forms() {
        let c6 = super::parse_hex_color("#1e2e3e");
        assert_eq!((c6.r, c6.g, c6.b, c6.a), (0x1e, 0x2e, 0x3e, 255));

        let c3 = super::parse_hex_color("#abc");
        assert_eq!((c3.r, c3.g, c3.b), (0xaa, 0xbb, 0xcc));

        let bad = super::parse_hex_color("garbage");
        assert_eq!((bad.r, bad.g, bad.b), (0, 0, 0));
    }

    #[test]
    fn document_view_produces_blank_frame() {
        let renderer = HeadlessRenderer::new();
        let view = View::Document {
            nodes: vec![DocumentNode::Paragraph {
                content: InlineText { text: "ignored".to_string() },
            }],
            style: DocumentStyle::default(),
        };
        let pixmap = renderer.render_to_pixmap(&view, 100, 100);
        // All pixels should be transparent (zero-filled Pixmap default).
        assert!(
            pixmap.pixels().iter().all(|p| p.alpha() == 0),
            "document view leaves blank canvas"
        );
    }
}
