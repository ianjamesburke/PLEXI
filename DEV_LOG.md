...
## 2026-04-18 — [CHANGED] V3 step 9e: FD CLOEXEC audit (→ v3)
New `src/fd_util.rs` with `set_cloexec(fd)` that shells out through `nix::fcntl::fcntl(fd, F_SETFD(FD_CLOEXEC | <existing flags>))` — Rust's stdlib sets `O_CLOEXEC` on `File::open` and `SOCK_CLOEXEC` on `UnixListener::bind` on Linux ≥1.80, but **not on macOS**, so we set `FD_CLOEXEC` explicitly. Three long-lived host FD sites now route through it: `typed_pipes::open_binary` (every binary pipe listener), `event_log::open_log_file` (workspace + global events.jsonl writer thread), `host::services::FileEventSink::new` (effects.jsonl writer). Audited via grep over `UnixListener::bind` / `UnixStream::connect` / `File::create` / `OpenOptions::new().open` — short-lived opens in `app_permissions::append_decision` and `plexi_iq::ledger::append_ledger` (open→write→drop on one call stack, no subprocess spawn in between) were left as stdlib-default. New Layer-1 test `fd_inheritance_test` (macOS-gated): binds a UnixListener + opens a JSONL file in the test process, spawns snake through Harness, shells out `lsof -p <child_pid> -Fn` and asserts neither path appears in the child FD table.
**Breaks if:** `lsof -p $(pgrep -f examples/snake/snake.py)` shows `/tmp/plexi-fd-audit-*.sock` in the output. `lsof -p <subprocess>` shows `~/.plexi-v3/effects.jsonl` or any workspace `.plexi/events.jsonl` path inherited into a child.
