...
- **Unrecoverable failure:** when the agent hits a wall it can't fix (context limit approaching, repeated test failures, architectural blocker), close the PR without merging (`gh pr close <n>`), comment on the original issue with what was tried and why it failed (under a `## Prior Attempts` section in the issue body), remove the `in progress` label, re-add the `ready` label, and exit. A fresh agent with clean context will do better than a thrashing one.
