...
**Spec gate:** Before writing the testing block, re-read the issue body's test plan (the `- [ ]` checklist under "Done When" or "Test plan"). Pass criteria must map 1:1 to items in that checklist. Do not add criteria for behaviors not listed in the spec, even if they seem like natural extensions of the feature. If no checklist exists, derive criteria only from the "Done When" section — never infer from the feature description.
...
- **Unrecoverable failure:** when the agent hits a wall it can't fix (context limit approaching, repeated test failures, architectural blocker), don't keep thrashing. Close the PR without merging (`gh pr close <n>`), comment on the original issue with what was tried and why it failed (under a `## Prior Attempts` section in the issue body), remove the `in progress` label, re-add the `ready` label, and exit the pane. A fresh agent with a clean context will do better than a exhausted one patching a broken attempt.
