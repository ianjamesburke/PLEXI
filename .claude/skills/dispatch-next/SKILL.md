...
- **CLI reference:** See the `/plexi-cli` skill for full `plexi terminal`, `plexi pane`, and `plexi open` usage — layout values (`split_h`, `split_v`, `split_right`, `split_below`, `tab`, `new_window`), flags, and patterns.
