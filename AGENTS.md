...
- **`just gen-cli-docs` / `just gen-config-docs` commit and push on their own.** Each recipe ends with `git add`, a conditional `git commit`, and `git push` — so running one inside a feature worktree silently lands a `chore(website): regenerate …` commit on your branch, then fails at the push if the branch has no upstream. The generation itself already succeeded by then. To regenerate without the commit, run the generator directly: `cargo run -p gen_config_docs > website/src/content/docs/config.md`. If a recipe already committed, `git reset --soft origin/alpha` folds it back into the working tree.
...
- **Runtime topology: your pane lives inside the BETA host.** Agent panes almost always run under `plexi-beta` (beta socket, `~/.plexi-beta/` profile), and a separate stable host may be running for Ian at the same time — a bare `plexi` call from your pane therefore targets *beta*, never stable and never your build under test. Alpha is not a running host by default: spawn one for end-to-end testing with `plexi host start` (see the `drive-host` skill). PR builds are ephemeral `plexi-pr-<N>` channels with their own profiles. The only unambiguous way to address a channel is the channel-named shim or an explicit `env PLEXI_CHANNEL=...` (leak rules in the next trap); orient with `echo $PLEXI_CHANNEL` before any command whose target channel matters.
- **`PLEXI_CHANNEL` leaks into the *bare* `plexi` binary.** A pane launched under beta runs `plexi app check` / `plexi app render` against the beta profile SDK even when the app path is under `.plexi-alpha/` — the bare binary names no channel of its own, so it adopts the pane's. A channel-named shim is authoritative and ignores the ambient variable: `plexi-alpha` always means alpha, `plexi-pr-<N>` always means that PR profile (`resolve_channel_dir` in `src/config/mod.rs`). For alpha validation use `plexi-alpha`, or `env PLEXI_CHANNEL=alpha plexi ...` if you only have the bare binary. Do not infer the runtime SDK/profile from the app path.
...
- **eframe skips `App::ui` entirely while the window is hidden.** `ViewportInfo::visible()` is false when the window is minimized *or fully occluded by other windows*; eframe then runs logic-only passes — `App::logic` fires, `App::ui` never does, and the pass still advances `cumulative_pass_nr`, so wakes "work" while nothing visible in `ui` executes. Every drain that services external clients (pane IPC, spawn queue, PTY events, shutdown, event-bus subscriptions, screenshot capture, Python pane runtime I/O — MCP server lines, HTTP replies, decoded guest messages via `service_python_pane_runtimes`, the hot-reload watcher's `drain_hot_reload_requests`) must live in `App::logic`, never in `ui`. Regression guards: `pane_ipc_serviced_while_window_hidden`, `mcp_reply_reaches_guest_while_window_hidden`; harness pump: `HostHarness::hidden_frame`. This also applies to *issuing* work, not just draining it: `plexi host screenshot` sent its `ViewportCommand::Screenshot` from `ui`, so an occluded host never asked for a capture — the PNG arrived only once the window was uncovered, after the CLI had timed out (`service_pending_screenshots`, stints 0495/0504). Python guest initialization, first-frame admission, and timer delivery also run off-paint through `LivePythonPane::background_tick`. Keep its wake predicate true for future deadlines, and bind the decoder repaint target from `service_external_io` before any paint. Otherwise a test that repeatedly pumps `HostHarness::hidden_frame` can pass while an idle real host never wakes to do the work.
- **"Not rendering" has two independent axes, and app panes are external clients on both.** An app answers assistant tool calls, raises notifications, and requests host actions on its own schedule — none of it caused by being painted — so app-pane servicing is external-client servicing and belongs in `App::logic` like every other drain above. It failed on *both* axes at once (stint 0688): the whole app-command drain lived in `App::ui`, so a minimized or occluded host executed no app command of any runtime; and `AppRuntime::Python`'s `background_tick` was an empty arm with `needs_background_tick` hardcoded `false`, so a Python pane in an inactive context was never serviced even on visible passes. A tool call reaches a guest through its stdin and is answered on its stdout with no render on either side, so both failures presented identically: the guest replied correctly and the host never read it, until `tool_dispatch`'s 30s timeout. When adding a runtime or a servicing path, satisfy both axes — `service_app_commands` for the occluded-window axis, `AppRuntime::background_tick`/`needs_background_tick` for the inactive-context axis. Guards: `app_commands_execute_while_window_hidden`, `python_tool_result_reaches_the_host_without_a_ui_pass`.
- **`rustfmt` on a mod-declaring file reformats every reachable module.** `rustfmt src/main.rs` rewrites dozens of unrelated files because rustfmt follows `mod` declarations. Alpha is not fmt-clean, so this pollutes the diff exactly like bare `cargo fmt`. Format per file with `rustfmt --check` and hand-apply, or format only leaf files you actually touched.
- **`cargo test --bin plexi` cannot see two classes of install break.** The justfile exports `RUSTFLAGS := "-D warnings ..."` only for `just` recipes, and test builds always compile with test callers present, so (1) an item whose last non-test caller was removed builds fine under `cargo test` and fails `just build` as dead code, and (2) a `cfg(all(target_os = "macos", not(test)))` path that no longer compiles is never compiled by `cargo test` at all. Both shipped green in PR #2594 and failed at `just pr-install`. `just build` is part of the pre-push gate for that reason; bare `cargo build` is not a substitute.
- **eframe destroys the window on close but does not end the process.** `EventResult::CloseRequested` (the X button, `ViewportCommand::Close`, the quit hotkey) runs `save_and_destroy` — `App::on_exit` and all — and then *waits*: eframe only calls `std::process::exit` on a later `EventResult::Exit`, which it can produce only from another winit window event arriving after the window is already gone. On Linux nothing need arrive, so the event loop parks in `epoll_wait` holding `notify.sock`, the PTY children and the wgpu worker threads, and `plexi host status` sees a live pid with `ready: false`. Ending the process is the host's job: every quit path finishes in `app::quit::exit_host`, the only place in the host that calls `std::process::exit`, and it unbinds the notify socket on the way out. Do not route `plexi host stop` through the window-close path to unify them — a hidden or occluded host runs no `App::ui` pass, so the close request would never be seen.
- **A quit path that exits from mid-frame skips `App::on_exit`.** `plexi host stop` exits from the render preamble, so anything `on_exit` does must also be done there explicitly or it silently stops happening: the focus journal survived every clean stop that way, and the next startup read it as a crash and emitted a `crash_recovery` segment measuring how long the host had been *off*. Shared step: `bank_final_focus_segment`.
- **`App::ui` is invisible to stint 0716's `[FREEZE]` heartbeat instrumentation.** That instrumentation lives entirely in `App::logic` and cannot see drag/resize stutter, which happens in `App::ui`'s widget-build cost — a live drag can stutter visibly with zero `[FREEZE]` lines. Use `PLEXI_UI_PROFILE=1` (`src/platform/ui_profile.rs`, stint 0731) to profile drag/resize frame cost by pane kind instead; it logs ranked `[UIPROF]` summaries under target `plexi::ui_profile` and costs nothing when unset.
