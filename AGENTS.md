...
- **eframe skips `App::ui` entirely while the window is hidden.** `ViewportInfo::visible()` is false when the window is minimized *or fully occluded by other windows*; eframe then runs logic-only passes — `App::logic` fires, `App::ui` never does, and the pass still advances `cumulative_pass_nr`, so wakes "work" while nothing visible in `ui` executes. Every drain that services external clients (pane IPC, spawn queue, PTY events, shutdown, event-bus subscriptions, screenshot capture, Python pane runtime I/O — MCP server lines, HTTP replies, decoded guest messages via `service_python_pane_runtimes`, the hot-reload watcher's `drain_hot_reload_requests`) must live in `App::logic`, never in `ui`. Regression guards: `pane_ipc_serviced_while_window_hidden`, `mcp_reply_reaches_guest_while_window_hidden`; harness pump: `HostHarness::hidden_frame`. This also applies to *issuing* work, not just draining it: `plexi host screenshot` sent its `ViewportCommand::Screenshot` from `ui`, so an occluded host never asked for a capture — the PNG arrived only once the window was uncovered, after the CLI had timed out (`service_pending_screenshots`, stints 0495/0504). Stint 0759 fixed two more: a WASM Python pane's guest `init` handshake, its *first* frame commit, and any due `SetTimer` fire all depend on state `LivePythonPane::ui` used to own exclusively — the initial `init` send needs a real `ui.available_size()`, and the frame commit and timer fire both need `frame_scheduler.poll_render` admission. A pane ticked only from `logic` (the active context under a hidden window, or any non-active/parked pane) never even told the guest its size, so `init(size, args)` never ran, `ready` never arrived, `lifecycle` stuck at `starting`, and a repeating timer froze the moment its pane stopped painting. `LivePythonPane::background_tick` now drives all three (`ensure_initialized` with a synthetic `HEADLESS_INIT_VIEWPORT` when no real size is known yet, then fire timers, poll, send, drain — the ordinary resize path corrects the synthetic size the instant a real paint happens) and `needs_background_tick` now also fires while uninitialized, ready-but-treeless, or a timer is due; `drain_all_app_commands`/`background_processes_need_wake` (`src/app/dispatch.rs`) tick the active context too when the window is hidden, since `ui()` then covers no pane at all, active or not.
