...
Do **not** hand-edit `CHANGELOG.md` for a release. It is generated by **git-cliff** from conventional commits when you cut a version.

- Preview: `just changelog`
- Cut a version on alpha (bumps `Cargo.toml`, regenerates CHANGELOG, commits, tags locally): `just bump` (patch/minor/major)
- Channel mechanics, alpha/beta tag schemes, and promote/release: [`scripts/RELEASE_CHANNELS.md`](scripts/RELEASE_CHANNELS.md) and [`scripts/AGENTS.md`](scripts/AGENTS.md)

Alpha dogfood tags are `vX.Y.Z-alpha.N` (see RELEASE_CHANNELS.md). `plexi-alpha update` only sees work that is on `alpha` **and** published as a GitHub Release tag the alpha channel accepts.


## v1 checklist + alpha dogfood (SSOT outside this repo)

Eng status and Ian Mac hand-verify live in one file (do not fork a second board):

`https://github.com/ianjamesburke/daily_log/blob/main/progress/plexi/V1-MASTER-CHECKLIST-2026-09-20.md`
